...
//                          List<String> ingredientsList, String tagsList, String imageUrl, int userId) throws SQLException {
	...
	public void addRecipe(String title, String description, int cookTimeMins, List<String> instructions,
	                      List<String> ingredientsList, String tagsList, File imageFile, int userId) throws SQLException {
        ...
        try {
            ...
            if (imageFile != null && imageFile.exists()) {
                insertRecipeImage(conn, recipeId, imageFile);
            }
    ...
    private void insertRecipeImage(Connection conn, long recipeId, File imageFile) throws SQLException {
        String sql = "INSERT INTO recipe_images (recipe_id, image_url, uploaded_at) VALUES (?, ?, NOW())";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setLong(1, recipeId);
            pstmt.setString(2, imageFile.getAbsolutePath());  // store full path as image_url
            pstmt.executeUpdate();
        }
    }
