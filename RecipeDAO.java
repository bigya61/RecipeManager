...
//
//    public void addRecipe(String title, String description, int cookTimeMins, List <String> instructions,
...
//    }
//
//    private long insertRecipe(Connection conn, String title, String description, int cookTimeMins,List <String> instructions, int userId) throws SQLException {
...
//            pstmt.setString(4, String.join("|", instructions));
...
//        try {
//            conn = DBConnector.getConnection();
//            String sql = "SELECT r.title, r.description,r.instructions, ri.image_url, r.cook_time_mins " +
//                         "FROM recipes r " +
...
//                    //List <String> instructions = rs.getString("instructions");
//                    String instructionsStr = rs.getString("instructions");
//                    List<String> instructions = instructionsStr != null ? List.of(instructionsStr.split("\\|")) : List.of("Instructions not available.");
//
//                    recipes.add(new RecipeData(title, description, imageUrl, null, cookTime,instructions));
//                }
//            }
//        } finally {
//            if (conn != null) conn.close();
...
//        String sql = "SELECT r.recipe_id, r.title, r.description,r.instructions, ri.image_url, r.cook_time_mins " +
//                     "FROM recipes r " +
//                     "LEFT JOIN recipe_images ri ON r.recipe_id = ri.recipe_id " +
...
//                List<String> instructions = instructionsStr != null ? List.of(instructionsStr.split("\\|")) : List.of("Instructions not available.");
//
//                RecipeData r = new RecipeData(title, description, imageUrl, ingredients, cookTime, instructions);
//                list.add(r);
//            }
//        }
//        return list;
...
public class RecipeDAO {
    ...
    // ✅ FIXED: This now fetches ingredients using recipe_id
    public static List<RecipeData> getTopRecipes(int limit) throws SQLException {
        List<RecipeData> recipes = new ArrayList<>();
        Connection conn = null;
        ...
        try {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                while (rs.next()) {
                    ...
                    List<String> instructions = instructionsStr != null && !instructionsStr.isEmpty()
                            ? List.of(instructionsStr.split("\\|"))
                            : List.of("Instructions not available.");

                    List<String> ingredients = fetchIngredients(recipeId);

                    recipes.add(new RecipeData(title, description, imageUrl, ingredients, cookTime, instructions));
                }
            }
        } finally {
            if (conn != null) conn.close();
    ...
    public static List<RecipeData> searchRecipes(String keyword) throws SQLException {
        ...
        try (Connection conn = DBConnector.getConnection();
            ...
            while (rs.next()) {
                ...
                String instructionsStr = rs.getString("instructions");
                List<String> instructions = instructionsStr != null
                        ? List.of(instructionsStr.split("\\|"))
                        : List.of("Instructions not available.");
